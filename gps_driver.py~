    ...
    def readloop(self):
        while True:
            ...
            try:
                ...
                if vals[0] == "$GPGGA":
			print("vals: ", vals[0])
			print("timestamp: ", vals[1])
			print("latitude: ", vals[2])
			#print("lat_bearing: ",  vals[3])
			print("longitude: ",  vals[4])
			#print("long_bearing: ",  vals[5])
			print("altitude: ",  vals[9])

			self.packet.timestamp = vals[1]
			self.packet.latitude = vals[2]
			#self.packet.lat_bearing = vals[3]
			self.packet.longitude = vals[4]
			#self.packet.long_bearing = vals[5]
			self.packet.altitude = vals[9]
			utm_coordinates = utm.from_latlon(self.packet.latitude, self.packet.longitude)
			self.packet.utm_x = utm_coordinates[0]
			self.packet.utm_y = utm_coordinates[1]
